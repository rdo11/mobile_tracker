...
def load_model(ckpt_path: str) -> tuple[nn.Module, list[str]]:
    ...
    if "fc.weight" in sd:                                  # resnet family
        ...
        # Discriminate by backbone width (classifier in_features):
        #   Tiny=768, Small=768, Base=1024, Large=1536
        width = sd["classifier.2.weight"].shape[1]
        if width == 1536:
            arch = "convnext_large"
        elif width == 1024:
            arch = "convnext_base"
        else:
            # 768: distinguish Tiny vs Small by depth of stage-5 (9 vs 27 blocks)
