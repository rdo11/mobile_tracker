#!/bin/bash
# after_v8.sh — overnight finisher. Runs AFTER training exits:
#   1. compare v5/v6/v8 on the real-crop holdout  -> /tmp/v8_compare.txt
#   2. deploy v8 ONLY if it beats v6 by >= +2 pts top-1 (safety margin)
#   3. append outcome to PROJECT_NOTES.md
#   4. put the Mac to sleep
set -u
cd "$HOME/Projects/mobile_tracker"
OUT=/tmp/v8_compare.txt

# 1) wait for training to end (never signals it)
while pgrep -f train_classifier.py > /dev/null; do sleep 60; done

{
  echo "=== v8 finished $(date) ==="
  tail -12 /tmp/train_v8.log
  echo
  SSL_CERT_FILE="$(.venv/bin/python -c 'import certifi; print(certifi.where())')" \
    .venv/bin/python compare_ckpts.py --data storage/dataset/dashcam_val \
      --ckpt models/r50_dashcam_v5_backup.pt models/r50_dashcam_v6.pt models/r50_dashcam_v8.pt
} > "$OUT" 2>&1

# 2) conditional deploy: parse top-1 of v6 vs v8 lines from the report
v6=$(grep -A3 "r50_dashcam_v6.pt" "$OUT" | grep -o "top-1 0\.[0-9]*" | head -1 | awk '{print $2}')
v8=$(grep -A3 "r50_dashcam_v8.pt" "$OUT" | grep -o "top-1 0\.[0-9]*" | head -1 | awk '{print $2}')
decision="KEPT v6 (v8 not clearly better or parse failed)"
if [ -n "${v6:-}" ] && [ -n "${v8:-}" ]; then
  better=$(.venv/bin/python -c "print(1 if $v8 >= $v6 + 0.02 else 0)")
  if [ "$better" = "1" ]; then
    cp models/r50_dashcam_v8.pt models/r50_dashcam.pt
    decision="DEPLOYED v8 (holdout $v8 vs $v6)"
  else
    decision="KEPT v6 (holdout $v8 vs $v6 — gain < 2 pts)"
  fi
fi
...
- v8 (305 cl, all new data) trained overnight; result + deploy decision:
  SEE /tmp/v8_compare.txt -> $decision
